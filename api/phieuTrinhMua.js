...
$(document).ready(function () {
    initDone.then(() => {
        ...
        if (phieuTrinhMuas.length <= 0) {
            getAllData(ptmPath).then((data) => {
                console.log(data);
                phieuTrinhMuas = data;
                phieuTrinhMuas.forEach(phieu => {
                    if (canDisplay(phieu)) {
...
function addRowToTable(data, index) {
    var ctpData = getChiTietPhieuById(data.idPhieuDeNghiMua);
    console.log(data)
    if (ctpData){
        var row = `
